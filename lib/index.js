 ...
 * 
 * This is actually the _default export_ of `@lumjs/not` but
 * is also available as a named `Engine` export (mostly for jsdoc).
 * 
...
class NoTengine
{
   ...
   * 
   * Will be passed the contents of any `{{ }}` blocks, regardless
   ...
   * 
   * @param {string} [options.dataVar="it"] Name of the data variable that
   * will be available in template blocks. I picked `it` as it was short,
...
def(NoTengine, 'Engine',   NoTengine);
def(NoTengine, 'Template', Template);
def(NoTengine, PS.self,    PS);
