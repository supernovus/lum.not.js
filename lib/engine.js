...
class NoTengine
{
   ...
   *
   * @param {(string|object)} id - Either the `id` of an existing template,
   * or an _options_ object like `render()` or `compile()` use.
   *
   * @returns {boolean}
   */
  remove(id)
  {
    if (isObj(id))
    {
      id = id.id ?? id.tmpl ?? id.text;
    }

    if (typeof id === S)
    {
      return this[PS.templates].delete(id);
    }
